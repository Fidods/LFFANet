...
class DemoLoader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # print(img)
        # image_array = np.array(img)  # 转换为NumPy数组
...
def save_result_for_test(dataset_dir, st_model, epochs, best_iou, recall, precision, true_positive_rate,
                         false_positive_rate, elapsed_time):
...
def make_dir(dataset, model):
    ...
    os.makedirs('./LFFANet/result/%s' % save_dir, exist_ok=True)  # Set save directory
    return save_dir
